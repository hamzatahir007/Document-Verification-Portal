...
const API = process.env.API_URL || 'http://localhost:5000/api/esahulat-officer';

const NAV = [
  {
    to: '/requests', label: 'Pending Approvals', icon: (
...
const Sidebar = ({ collapsed = false, onToggle }) => {
  ...
  return (
    <>
      ...
      <aside className="wb-sidebar" style={{ ...s.sidebar, width: collapsed ? 64 : 240 }}>
        {/* Brand + Toggle */}
        <div style={{ ...s.brandRow, padding: collapsed ? '20px 0' : '20px 20px', justifyContent: collapsed ? 'center' : 'space-between' }}>
          {!collapsed && (
            <div style={s.brand}>
              <div style={s.brandMark}><span style={s.brandLetter}>V</span></div>
              <div>
          ...
          <button className="wb-toggle" onClick={onToggle} style={{ ...s.toggleBtn, margin: collapsed ? '0 auto' : '0' }} title={collapsed ? 'Expand' : 'Collapse'}>
            {collapsed ? (
        ...
        {collapsed && <div style={s.brandMarkCollapsed}><div style={s.brandMark}><span style={s.brandLetter}>W</span></div></div>}
        ...
        <div style={{ padding: collapsed ? '10px 8px 20px' : '10px 10px 20px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          <button
            className="wb-logout"
            onClick={logout}
